...
static int vfio_intx_enable(VFIOPCIDevice *vdev, Error **errp)
{
    ...
    if (!pin) {
#if OSNET_HOTPLUG_VFIO_PRINT 
      printf("%s: PCI_INTERRUPT_PIN\n", __func__);
#endif
      return 0;
    }

#if OSNET_HOTPLUG_VFIO_PRINT 
    printf("after return in %s\n",__func__);
#endif
...
void vfio_realize(PCIDevice *pdev, Error **errp)
#else
...
#if OSNET_HOTPLUG_VFIO
void vfio_pci_reset(DeviceState *dev)
#else
...
static const VMStateDescription vfio_pci_vmstate = {
    .name = "vfio-pci",
    /* OSNET-HOTPLUG-VFIO
     * Setting unmigratable to false on the source and
     * destination results in the migration failure. Length
     * mismath 0x3000 != 0x4000. Error while loading the state

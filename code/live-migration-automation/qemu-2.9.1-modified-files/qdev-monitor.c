...
DeviceState *osnet_qdev_device_add(QemuOpts *opts, Error **errp)
{
    ...
    /* OSNET-HOTPLUG-VFIO: Realize the device object at the
     * later time.
     */
#if 0
    object_property_set_bool(OBJECT(dev), true, "realized", &err);
...
void qmp_osnet_device_add(QDict *qdict, QObject **ret_data, Error **errp)
{
    ...
    dev = osnet_qdev_device_add(opts, &local_err);
    if (!dev) {
        error_propagate(errp, local_err);
        qemu_opts_del(opts);
        return;
...
void qmp_osnet_vfio_nic_realize_top_half(QDict *qdict, QObject **ret_data, Error **errp)
{
    ...
    osnet_device_set_realized_top_half(OBJECT(dev), true, &err);
    dev->realized = true;
}
...
void qmp_osnet_vfio_nic_realize_bottom_half(QDict *qdict, QObject **ret_data, Error **errp)
{
    ...
    const char *id = qdict_get_str(qdict, "id");
    DeviceState *dev = find_device_state(id, &err);
    osnet_device_set_realized_bottom_half(OBJECT(dev), true, &err);
}
#endif

...
void hmp_osnet_vfio_nic_create(Monitor *mon, const QDict *qdict)
{
    //printf("%s\n", __func__);
    Error *err = NULL;

    qmp_osnet_device_add((QDict *)qdict, NULL, &err);
    const char *id = qdict_get_str(qdict, "id");
    DeviceState *dev = find_device_state(id, &err);
    if (!dev) {
      hmp_handle_error(mon, &err);
    }
}

void hmp_osnet_vfio_nic_realize_top_half(Monitor *mon, const QDict *qdict)
{
    //printf("%s\n", __func__);
    Error *err = NULL;

    qmp_osnet_vfio_nic_realize_top_half((QDict *)qdict, NULL, &err);

    hmp_handle_error(mon, &err);
}

void hmp_osnet_vfio_nic_realize_bottom_half(Monitor *mon, const QDict *qdict)
{
    //printf("%s\n", __func__);
    Error *err = NULL;

    qmp_osnet_vfio_nic_realize_bottom_half((QDict *)qdict, NULL, &err);

	  hmp_handle_error(mon, &err);
}

void hmp_osnet_vfio_nic_reset(Monitor *mon, const QDict *qdict)
{
    Error *err = NULL;
    const char *id = qdict_get_str(qdict, "id");
    DeviceState *dev = find_device_state(id, &err);
    if(dev->hotplugged){
    	osnet_device_reset(dev);
    }
}
#endif
/* OSNET-END */

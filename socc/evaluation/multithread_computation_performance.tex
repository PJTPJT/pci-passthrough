...
compared to the bare-metal performance.
%\textbf{VANILLA}: unmodified guest has the configuration without our optimizations.
}

\figw{fpspeed_barchart}{8.5}{
Slowdown of floating point computation benchmarks in the
SPEC CPU 2017~\cite{bucek:2018} suite for VANILLA and 
DID guests compared to the bare-metal performance. 
...
Slowdown of integer computation benchmarks in the
SPEC CPU 2017~\cite{bucek:2018} suite for VANILLA and 
DID guests compared to the bare-metal performance. 
The x-axis refers to the IDs for the following benchmarks:
%\textbf{VANILLA}: unmodified guest has the configuration
%without our optimizations. 
\textbf{500}: perl scripts.

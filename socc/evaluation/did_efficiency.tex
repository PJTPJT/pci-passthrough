...
\hline
& \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}B/W\\ (Gbps)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}CPU\\ Core\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}NIC INTR\\ (per sec)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}TMR INTR\\ (per sec)\end{tabular}} \\ \hline
\multirow{2}{*}{Bare-Metal} & \multirow{2}{*}{37.39} & 0 & 118592 & 256 \\ \cline{3-5}
&  & 1 & 0 & 348 \\ \hline
\multirow{2}{*}{DTID} & \multirow{2}{*}{37.35} & 0 & 110856 & 255 \\ \cline{3-5}
&  & 1 & 0 & 348 \\ \hline
\end{tabular}
\end{center}
\vspace{-0.05in}
...
To evaluate the usefulness of direct timer interrupt delivery,
we measured the timer interrupt latency by setting up a
periodic timer in the kernel. The timer interrupt latency was
the timing difference between the expected and actual wakeup
time. We ran the experiment with the sleep time of 200$\mu$s
...
the bare-metal OS and reduced the median timer interrupt
latency by four times over a VANILLA HaaS VM. Because a DTID
or DID HaaS VM still experienced VM exits due to reasons such
as privileged instructions, its median timer interrupt latency
is 0.52$\mu$s more than the bare-metal OS. Reducing this
difference further by identifying the underlying VM exit
reasons is part of our future work.
...
interrupt was serviced and the timer was reset. Nonetheless,
...
correctly. Although the DTID configuration incurred additional

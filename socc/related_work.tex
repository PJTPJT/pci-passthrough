...
in the VM for each type of physical device, besides active 
interception of these operations by the hypervisor. 
Recently proposed VFIO Passthrough~\cite{vfio-passthrough} framework
in KVM/QEMU provides a generic framework for guest device drivers 
to save and restore physical device states, which requires modifying
individual device drivers to become migration aware.
In contrast, \sna's live migration mechanism is device agnostic; 
device drivers in Haas VM do not need
device-specific state capture and reconstruction code 
and the \na does not need to intercept guest I/O accesses.
Finally, unlike the above approaches,
\na also supports posted timer interrupts, posted IPIs,
and live migration of guests having direct access to timer hardware and PCIe devices.

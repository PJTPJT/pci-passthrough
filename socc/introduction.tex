...
access to resources, the SVMvisor is designed to maximize the HaaS VM's 
access to physical hardware while retaining the ability to live migrate the VM on-demand.
Specifically, SVMvisor provides the following features for a HaaS VM. 
\begin{itemize} 
%\setlength\itemsep{-0.04in}
\parskip 0mm
\itemsep 0mm

\item Dedicated physical CPU and memory resources for HaaS VM.

\item Direct access to all PCIe devices and local APIC timers by HaaS VM without hypervisor involvement.

\item Direct delivery of all interrupts. Existing approaches~\cite{posted-interrupts,others}
only deliver PCIe device interrupts directly to the guest. In addition, SVMvisor also supports 
direct delivery of hardware timer interrupts from local APIC  and inter-processor interrupts (IPI) between 
virtual CPUs (VCPUs) without hypervisor intervention via a novel use of VT-d 
posted-interrupt~\cite{posted-interrupt} mechanism.

\item Elimination of all major VM Exit overheads  during a HaaS VM's execution. 
We identify how, in traditional passthrough I/O support,  VM Exits still
lower the I/O throughput and increase CPU usage of the host.
SVMvisor includes mechanisms to systematically eliminate
these overheads to match bare-metal I/O performance and CPU usage within a HaaS VM.

\item Seamless live migration of a HaaS VM having passthrough access to physical devices and local APIC timers. 
Unlike existing approaches for bare-metal live migration~\cite{blmvisor,others}, 
SVMvisor does not use any device-specific migration code, and yet matches the 
liveness and application performance metrics of traditional live migration.
Except during VM migration, SVMvisor is not involved in the HaaS VM's normal execution 
and thus imposes a negligible performance overhead at run time.  

\item Small execution and memory footprint of SVMvisor, currently limited to one non-dedicated CPU core and around 100MB of RAM, with room for further reduction.
...
We first describe the design and implementation of SVMvisor,
specifically the detailed description of the above contributions.
Next, we evaluate the performance of our SVMvisor prototype

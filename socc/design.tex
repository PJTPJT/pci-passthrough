...
\figw{architecture}{8.75}{Architecture of \na for single-VM virtualization:
The \na runs as a thin hypervisor that provides
manageability services such as live migration and VM monitoring.
The HaaS VM runs on all the remaining CPU cores with direct access 
to I/O devices and local timers, including direct delivery of timer, 
device, and inter-processor interrupts.}

Figure~\ref{fig:architecture} shows the high-level architecture of \na.
\na runs as a thin hypervisor on each physical server and 
supports a single VM, called the HaaS VM, on which 
a users installs preferred OS and applications. 
\na primarily provides value-added functionalities, such as 
live migration, introspection, and performance monitoring, 
while staying out of normal execution path of a HaaS VM.
...
\figw{nic_bonding}{9}{Bonding of pass-through and para-virtual NICS in a HaaS VM to support live migration in \na.}

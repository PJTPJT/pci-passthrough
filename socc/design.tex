...
(such as emulated floppy disks and CD-ROMs); these
tend to communicate with QEMU by trapping on programmed I/O instructions
that result in VM exits.
For accessing local APIC timer (as explained later), 
we also disable VM exits due to {\em MSR
write} operations to local APIC's initial count and divide configuration
registers. For sending IPIs between VCPUs, 
we disable VM exits due to MSR writes to
...
interrupting the HaaS VM on other CPUs.
...
interrupt into a PIN interrupt and allows the HaaS VM to directly update the PID page.}
...
Therefore, such local interrupts cannot be delivered to a VM via the traditional posted interrupt mechanism.

%TODO: The following description of direct interrupt delivery seems too short. Could use more explanation.
{\bf Direct Timer Interrupt Delivery:} 
To enable a timer interrupt to be delivered directly to a HaaS VM's VCPU running on a CPU core,
\na first configures the {\em local vector table} in the local APIC to turn each timer interrupt into a PIN interrupt. 
Next, it programmatically sets a particular bit in the CPU core's PIR bitmap to indicate to the HaaS VM that underlying the 
PIN interrupt is in fact a timer interrupt. Thus, when the local APIC on the CPU core delivers a timer interrupt,

...
%\subsubsection{Virtual Function I/O}

Virtual function I/O (VFIO) is a Linux software framework that enables user-space
device drivers to interact with PCIe devices directly without involving the Linux kernel. 
Essentially, VFIO makes the configuration register space and the memory regions specified in the BARs of
...
Typically, a PCIe device driver communicates with its device using PIO or
...
In the conventional non-DID architecture, when an interrupt is delivered to a CPU core,
...
In the X86 architecture, a VM may experience two types of interrupts: {\em external} and {\em local} interrupts.
...
When a VM completes the service of an interrupt, it needs to clear the EOI (End of Interrupt) register to indicate to the 
...
\subsubsection{Direct Local Interrupt Delivery} \label{subsubsec:shared_pid_dtid}
...
which causes a VM exit. To avoid this VM exit, \na leverages VT-d's hardware-assisted APIC virtualization by properly configuring the MSR bitmap of the associated VMCS. 
As a result, the KVM's  intercept of any TMICT MSR update is disabled. When a VM writes to the TMICT, the change is written to the associated register
directly without triggering any VM exit.     

Combining all the above techniques, \na is able to deliver local timer interrupts, periodic or aperiodic, directly to a VM running on the associated CPU core and have them properly serviced, all 
...
Because the states associated with directly accessed devices complicate the migration process~\cite{zhai:2008}, 
\na augments KVM's standard VM migration capability with additional mechanisms to support seamless migration of HaaS VMs. 
...
and then teams them up using the active-backup mode of the Linux bonding driver~\cite{bonding}, as shown in Figure~\ref{fig:nic_bonding}.
...
migration~\cite{clark:2005}, all the memory pages are
...
After the HaaS VM is successfully migrated and resumed, both the hypervisor and the VM restart 
the DTID mechanism guest, with the steps being the reverse of

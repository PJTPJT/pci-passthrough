...
In \sna, a CPU core's PID is accessible to the IOMMU (Interrupt Remapping Table), the hypervisor and the HaaS VM running on the CPU core. 
%Originally, the PID  is embedded within the HaaS VM's VMCS structure.
To make a PID accessible to its associated HaaS VM,
when a vCPU is created, \na allocates a separate page for its PID, and places the PID's base address in the vCPU's VMCS, and in all the Interrupt Remapping Table entries that target at  the vCPU.
